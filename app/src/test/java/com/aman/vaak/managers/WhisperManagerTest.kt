...
class WhisperManagerTest {
    ...
    inner class ConfigurationTests {
        ...
        fun `throws configuration error when API key missing`() {
            ...
            val exception =
                assertThrows(TranscriptionException.ConfigurationError::class.java) {
    ...
    inner class TranscriptionTests {
        @Test
        fun `returns success with TranscriptionResult when transcription succeeds`() =
            runTest {
        ...
        @Test
        fun `returns failure with NetworkError when API fails`() =
            runTest {
                whenever(fileManager.isFileValid(testFile)).thenReturn(true)
                whenever(fileManager.createFileSource(testFile)).thenReturn(fileSource)
        ...
        @Test
        fun `uses language override when provided`() =
            runTest {
                whenever(fileManager.isFileValid(testFile)).thenReturn(true)
                whenever(fileManager.createFileSource(testFile)).thenReturn(fileSource)
                ...
                verify(openAI).transcription(
                    argThat { request: TranscriptionRequest ->
                        request.language == "fr"
                    },
                )
            }
        ...
        @Test
        fun `uses config language when no override provided`() =
            runTest {
                whenever(fileManager.isFileValid(testFile)).thenReturn(true)
                whenever(fileManager.createFileSource(testFile)).thenReturn(fileSource)
                whenever(openAI.transcription(any())).thenReturn(Transcription("text"))
        ...
        @Test
        fun `returns failure with FileError when file is invalid`() =
            runTest {
