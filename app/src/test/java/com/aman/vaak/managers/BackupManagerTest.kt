...
class BackupManagerTest {
    ...
    inner class `createBackup` {
        @Test
        fun `creates backup successfully`() = TestScope(StandardTestDispatcher()).runTest {
        ...
        @Test
        fun `handles file write error with storage exception`() = TestScope(StandardTestDispatcher()).runTest {
        ...
        @Test
        fun `handles JSON serialization error`() = TestScope(StandardTestDispatcher()).runTest {
            whenever(moshi.adapter(Backup::class.java)).thenReturn(backupAdapter)
            whenever(promptsManager.getPrompts()).thenReturn(emptyList())
            whenever(settingsManager.getTargetLanguage()).thenReturn(Language.ENGLISH)
            whenever(settingsManager.getFavoriteLanguages()).thenReturn(listOf(Language.ENGLISH))
    ...
    inner class `restoreBackup` {
        @Test
        fun `handles invalid backup format`() = TestScope(StandardTestDispatcher()).runTest {
        ...
        @Test
        fun `handles file read error`() = TestScope(StandardTestDispatcher()).runTest {
            val backupFile = File("/path/to/backup.json")
        ...
        @Test
        fun `handles JSON parsing error`() = TestScope(StandardTestDispatcher()).runTest {
            val backupFile = File("/path/to/backup.json")
    ...
    inner class `backup functionality` {
        @Test
        fun `uses current version in backup`() = TestScope(StandardTestDispatcher()).runTest {
            val mockFile = File("/downloads/test.json")
            ...
            verify(backupAdapter).toJson(org.mockito.kotlin.argThat { backup ->
                backup.version == Backup.CURRENT_VERSION
            })
        }

        @Test
        fun `generates timestamped filename`() = TestScope(StandardTestDispatcher()).runTest {
            val mockFile = File("/downloads/vaak_settings_123456.json")
            ...
            verify(fileManager).getDownloadsFile(org.mockito.kotlin.argThat { filename ->
                filename.startsWith("vaak_settings_") && filename.endsWith(".json")
            })
        }

        @Test
        fun `collects all manager data for backup`() = TestScope(StandardTestDispatcher()).runTest {
            val mockFile = File("/downloads/test.json")
    ...
    inner class `error handling` {
        @Test
        fun `handles storage exception with null message`() = TestScope(StandardTestDispatcher()).runTest {
            val mockFile = File("/downloads/test.json")
        ...
        @Test
        fun `createBackup returns failure when promptsManager fails`() = TestScope(StandardTestDispatcher()).runTest {
